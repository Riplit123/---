...
document.addEventListener('DOMContentLoaded', function() {
    ...
    let desktopIcons = [
        { name: 'Установщик антивируса', app: 'installer' },
        { name: 'Документы', app: 'documents' },
        { name: 'Projects', app: 'projects' },
        { name: 'Секретно', app: 'secret' },
   ...
   // Обновление часов
    ...
    function createWindow(app, titleOverride) {
        ...
        // Если это окно myApp, запускаем автоматическую авторизацию
        if (app === 'myApp') {
            simulateAutoLogin(windowElement);
        }
    ...
    function getAppName(app) {
        switch(app) {
            ...
            case 'account':
                return 'Аккаунт';
            case 'myApp':
                return 'Антивирус KvantDefender';
            default:
                return app || 'Файл';
        }
    }
  ...
  // Функция получения содержимого приложения
    ...
    function getTextFileContent(fileName) {
        ...
        switch(fileName) {
            case 'Заметки.txt':
                content = '<pre>Памятка:\n- Обновить антивирус на рабочем столе\n- Авторизоваться\n- Сделать резервное файлов\n</pre>';
                break;
            case 'Notes.txt':
            ...
            case 'Отчет.doc':
            case 'Report.doc':
                content = '<p>Финансовый отчет за квартал.</p>';
                break;
            case 'unknown.exe':
                content = '<p>Неизвестное исполнение.</p>';
                break;
            case 'LoginInfo.txt':
                content = '<pre>Логин: админ\nПароль: 04514221\n</pre>';
                break;
            case 'пароли.txt':
                content = '<pre>Логин: админ\nПароль: 04514221\n</pre>';
                break;
    ...
    function getSecretContent(windowId) {
        return `
            <div class="password-prompt">
                ...
                <p>Подсказка: Чем больше из меня берут, тем больше я становлюсь. Что я?</p>
   ...
   // Содержимое "Установщика программы"
    function getInstallerContent() {
        return `
            <div id="installer">
                <div class="installer-step" id="step-1">
                    <h2>Добро пожаловать в установщик антивируса!</h2>
                    <p>Этот мастер поможет вам установить защиту на ваш компьютер.</p>
                ...
                <div class="installer-step hidden" id="step-2">
                    ...
                    <p>Чтобы продолжить, нажмите кнопку "Далее", нажав на колесико мыши.</p>
                    <textarea rows="10" cols="50">ЛИЦЕНЗИОННОЕ СОГЛАШЕНИЕ НА ПОЛЬЗОВАНИЕ ПРОГРАММНЫМ ОБЕСПЕЧЕНИЕМ «УНИКАЛЬНЫЙ СЕРВИС»

Пожалуйста, внимательно ознакомьтесь с условиями данного Соглашения перед установкой Программного Обеспечения. Продолжение установки означает ваше согласие со всеми нижеперечисленными пунктами.

1. Право на использование
Вы имеете право использовать данное программное обеспечение исключительно для личных нужд, если вы не являетесь инопланетным существом, киборгом или роботом-пылесосом. В случае обнаружения нарушения этого пункта, мы оставляем за собой право отключить ваш компьютер прямо посреди рабочего дня без предупреждения.

2. Ограничения использования
Вам строго запрещено:
- Использовать программу для управления межгалактическими кораблями;
- Попытаться взломать код программы, чтобы сделать себе вечную молодость;
- Продавать, дарить или передавать права на использование программы своим друзьям, соседям или домашним питомцам;
- Разговаривать с программой вслух, даже если она вам кажется умной.

3. Ответственность сторон
Мы не несём ответственности за любые повреждения вашего компьютера, возникшие вследствие неправильного использования программы, включая, но не ограничиваясь:
- Случайный клик правой кнопкой мыши вместо левой;
- Забывание пароля к программе, потому что он был записан на стикере, который съел кот;
- Неправильное подключение кабеля питания, когда программа была запущена.

4. Конфиденциальность
Мы гарантируем, что все ваши данные будут храниться в полной безопасности, если только наша база данных не будет захвачена армией злых роботов. В этом случае мы рекомендуем немедленно бежать и прятаться в ближайшем укрытии.

5. Условия расторжения договора
Вы можете прекратить использование программы в любой момент, просто удалив её с вашего устройства. Однако помните, что удаление программы не освобождает вас от обязательств по данному соглашению, особенно если вы нарушили какой-либо из пунктов выше.

6. Дополнительные условия
Если вы вдруг обнаружите, что программа начала говорить с вами голосом Дарта Вейдера, пожалуйста, немедленно свяжитесь с нами. Мы постараемся решить эту проблему как можно скорее.

7. Завершение установки
Для завершения установки и принятия всех условий данного Лицензионного Соглашения нажмите на кнопку "Далее" колёсиком вашей мышки!</textarea>
                    <div class="installer-buttons">
                        <button id="license-next-button">
                        Далее</button>
                    </div>
                    <div id="license-hint"></div>
                </div>
                <div class="installer-step hidden" id="step-3">
                    <h2>Выбор пути установки</h2>
                    <p>Выберите папку, в которую будет установлена программа.</p>
                    <input type="text" value="C:\\Program Files\\MyProgram">
                    <div class="installer-buttons">
                ...
                <div class="installer-step hidden" id="step-4">
                    <h2>Установка</h2>
                    <p>Идет установка программы, пожалуйста, подождите...</p>
                    <div id="progress-bar">
                ...
                <div class="installer-step hidden" id="step-5">
                    ...
                    <p>Программа успешно установлена.</p>
                    <div class="installer-buttons">
  ...
  // Функции установщика программы
    ...
    // Доработка для лицензионного соглашения
    document.addEventListener('click', function(event) {
        ...
        if (target && target.id === 'license-next-button') {
            ...
            if (event.button === 2) {
                ...
                hint.textContent = 'Стоит прочитать лицензионное соглашение до конца :)';
            }
        }
    });
    ...
    function simulateAutoLogin(windowElement) {
        ...
        setTimeout(function() {
            ...
            loginForm.addEventListener('submit', function(e) {
                ...
                if (usernameInput === 'админ' && passwordInput === '04514221') {
                    loginErrorDiv.classList.add('hidden');
                    appContentDiv.classList.remove('hidden');

                    // После успешной авторизации показываем загрузку
                    setTimeout(function() {
                        alert('Файлы антивируса повреждены. Необходимо исправить код криптозащиты, чтобы защитить данные!');
